...
-- не связаны с dbo.images. Однако если бы были, то вот как бы я это реализовал:
--SELECT *
--FROM users u 
...
-- Опять таки, таблицы dbo.alboms и dbo.users
-- не связаны с dbo.images. Но вот как я это реализовал бы
--SELECT *
--FROM albums a
...
--);  - что-то в таком духе. На самом деле, тяжело предпологать правильную реализацию, ибо если один альбом
	--  может одновременно иметь несколько картинок(их ссылки или айдишники), то это нужно хранить в какой то структуре
	--  (первое, что приходит в голову - json. С технической точки зрения могу ошибаться, поскольку не знаю все возможные
	--   варианты и, возможно, json - не самая лучшая идея.) - но это уже, что называется, экстрополяция (extrapolation).
